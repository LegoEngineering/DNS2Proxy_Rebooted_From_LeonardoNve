...
def Task_1():
    ...
    for line in input:
      for i in range(len(line)):
        if line[i:i+4] == "AAAA": #AAAA arrives only once during the connection, making it a good indicator
          ...
          timestamp.append(line[11:20]) #create a list  of timestamps
    ...
    for i in range(len(timestamp)):
      ...
      seconds = long(timestamp_elements[2][:len(timestamp_elements[2])-1]) * 1000
      time.append(hours + minutes + seconds) #combine the times and add them to a list of times
    ...
    for line in connection_log:
      ...
      if line[58:61] == 'www' and delta_t >= 1000: #if the difference in time is greater than just under a minute, and is not the name of the host...
 	hosts.append(line)      
