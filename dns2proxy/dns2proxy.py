...
def requestHandler(address, message):
    resp = None
    dosleep = False
    unwanted_hosts=[] #declare array for blocked site list
    with open("domains.cfg","r") as unwanted_hosts_file: #open the file containing the blocked site list
	for host in unwanted_hosts_file: #iterate through the lines of the file
		if len(host)>1:
			p = host.split(" ")[0]  #claim the first element in each line, as divided by a space, as a new variable
			unwanted_hosts.append(p[1:]) #omit the period at the beginning of each host name and store in the array
    try:
        ...
        try:
            ...
            try:
                ...
                if op == 0:
                    ...
                    if len(qs) > 0:
                        q = qs[0]
			domain_request =str(q) #move the domain request into its own variable
			block=0 #set block to 0 because sites are unblocked unless explicitly made otherwise
			for d in unwanted_hosts: #iterate through the list of blocked sites
				if (domain_request.find(d)>=0): #if the request contains any member of the list of blocked sites...
					block=1 #set blocking to 1
			if (block==0): #if blocking is not on...
			    DEBUGLOG('request is ' + str(q))
	                    save_req(LOGREQFILE, 'Client IP: ' + address[0] + '    request is    ' + str(q) + '\n') #announce the handshake data in the logfile
